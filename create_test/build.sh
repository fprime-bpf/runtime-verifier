...
for target in "${TARGETS[@]}"; do
  for cache in "${CACHES[@]}"; do
	...
	filehandle="rolled_${target}_${cache}"
    "${PREFIX}/clang" -emit-llvm -S "${filehandle}.bpf.c" -O0 -target bpf -mcpu=duotronic -fno-unroll-loops
    "${PREFIX}/llc" -march=bpf -mcpu=duotronic -filetype=obj -o "${filehandle}.bpf.o" -O0 -bpf-stack-size=40000 "${filehandle}.bpf.ll"
    "${PREFIX}/llvm-objcopy" -O binary --only-section=.text "${filehandle}.bpf.o" "../test/${filehandle}.o"


	filehandle="unrolled_${target}_${cache}"
    "${PREFIX}/clang" -emit-llvm -S "${filehandle}.bpf.c" -O0 -target bpf -mcpu=duotronic -fno-unroll-loops
    "${PREFIX}/llc" -march=bpf -mcpu=duotronic -filetype=obj -o "${filehandle}.bpf.o" -O0 -bpf-stack-size=1000000 "${filehandle}.bpf.ll"
    "${PREFIX}/llvm-objcopy" -O binary --only-section=.text "${filehandle}.bpf.o" "../test/${filehandle}.o"
  done
done
